...
function ShiftDetailPanel({
  ...
  return (
    <div className="sch-detail-panel">
      {/* Header */}
      ...
      {/* Employee */}
      <div className="sch-detail-employee">
        <div className="sch-task-avatar" style={{ width: 28, height: 28, fontSize: 10 }}>{employeeInitials}</div>
        <span className="sch-detail-employee-name">{employeeName}</span>
      </div>

      {/* Checklist section */}
      <div className="sch-detail-section-header">
        <span className="sch-detail-section-title">Checklist</span>
        {checklist.length > 0 && (
          <span className="sch-detail-progress-pill">{completedCount}/{checklist.length} done</span>
        )}
      ...
      {showAddForm ? (
        <div className="sch-detail-add-form">
          ...
          <div className="sch-detail-form-row">
            <div className="sch-detail-type-toggle">
              <button
                className={`sch-detail-type-btn${newType === 'yes-no' ? ' active' : ''}`}
                onClick={() => setNewType('yes-no')}
              >Yes / No</button>
              <button
                className={`sch-detail-type-btn${newType === 'rating' ? ' active' : ''}`}
